...
## 2805 농작물 수확하기
특정 규칙에 따라 수확한 농작물의 수익을 구하는 문제

마름모 형태로 수확을 할 수 있다.

가장 위쪽과 아래쪽부터 양옆에 수확하지 못하는 칸의 개수를 빼주면서 총 수익값을 구해주었다.

리스트를 뒤에서부터 인덱스 할때는 `-`에 1를 더해주어야한다.
> 뒤에서 0번째 인덱스 `리스트[-1]`


## 9367 점점 커지는 당근의 개수
점점 커지는 연속된 당근의 최대 개수를 구하는 문제

2번째 당근부터 크기를 비교하여 연속할 때는 개수를 증가시키고 불연속 하면 초기화 시켜서 연속된 당근의 개수를 구했다.
...
## 10760 우주선착륙2
높이가 자신보다 낮은 지점의 개수가 4개 이상인 예비 후보지 개수를 구하는 문제

모든 지점을 8방향 델타 탐색 하면서 예비 후보지 개수를 구한다.


## 1258 행렬찾기
행렬안에서 0이 아닌 숫자들로 이루어진 부분 행렬을 찾아 크기가 작은 행렬부터 순서대로 출력하는 문제

부분행렬을 찾기위한 시작점의 규칙은 자신은 0이 아니고 위쪽과 왼쪽의 요소는 0이어야 한다.

부분행렬을 찾기위한 끝점의 규칙은 자신은 0이 아니고 아래쪽과 오른쪽의 요소는 0이어야 한다.

이 규칙을 모든 점에 적용하기 위해 위와 아래에는 0으로 이루어진 리스트를 왼쪽과 오른쪽에는 0의 요소를 하나씩 추가한 2차원 리스트를 만들었다.

모든 점을 순회하면서 부분 행렬을 찾아준다.

행렬을 찾은 뒤 크기에 따라 정렬해줘야 하는데 버블 정렬을 이용하여 정렬해주었다.


## 6019 기차 사이의 파리
두 기차 사이에서 움직이는 파리가 이동한 거리를 구하는 문제

`거리 = 속력 * 시간`

`시간 = 거리 / 속력`

파이썬에서 나눗셈은 근사값으로 결정한다. -> 오차가 발생

나눗셈을 하고 곱셈을 하면 오차가 더 커진다


## 16910 원 안의 점
반지름 N인 원안의 격자점의 개수를 구하는 문제

x축과 y축에 있는 격자점은 4N + 1 개이다. 

1사분면에 격자점만 조건을 비교하여 4를 곱해주었다
- 각 사분면의 격자점이 대칭이기 때문에
- `x**2 + y**2 <= N**2`


## 3499 퍼펙트 셔플
카드를 퍼펙트 셔플한 결과를 출력하는 문제

카드 개수가 홀수라면 빈 카드를 더해서 짝을 맞춰주는 것이 포인트!


## 10580 전봇대
전봇대에 연결된 전선의 교차점의 개수를 구하는 문제

정렬이 필요하다

밑에 줄과 위에 줄을 모두 비교하여 교차하면 개수를 추가


## 5188 최소합
0, 0 에서 N, N까지 오른쪽과 아래쪽 방향으로만 이동할 수 있을 때 최소합을 구하는 문제

완전탐색과 가지치기를 활용한다.

재귀함수를 사용할 때 합을 추가로 넣어줘 조건문을 사용한다.

- 시간초과가 나올시 조건문을 다시 살핀다.


## 5189 전자카트
사무실에서 관리구역을 갈때 사용되는 배터리의 최솟값을 구하는 문제

관리구역을 가는 경로를 순열로 만든다

사무실에서 출발하여 사무실로 돌아오기 때문에 각 관리구역을 이동할 때 사용하는 배터리 양을 전부 구한다

그 중 최솟값을 출력한다.


## 5202 화물도크
24시간 이내에 작업할 수 있는 화물차 최대 개수를 구하는 문제

우선 화물자의 시작시간과 종료시간을 담은 리스트를 정렬한다

처음 시작시간과 종료시간을 정하고 다음 신청서와 비교하면서 갱신해준다

만약 다음 신청서의 종료시간이 더 짧다면 현재신청서를 다음신청서로 바꿔준다

다음 신청서의 시작시간이 현재 신청서의 종료시간 이상이라면 화물차 개수를 증가시키고 시작, 종료시간을 갱신한다


## 5201 컨테이너 운반
트럭을 사용하여 옮길 수 있는 컨테이너 무게의 최댓값을 구하는 문제

가벼운 것부터 트럭으로 옮긴다면 더 무거운 것을 옮길 수 있는 트럭을 사용해버릴 수 있기 때문에 무거운 것부터 트럭으로 옮기는 전략을 사용

트럭의 적재용량이 무거운 것부터 하기 때문에 다음 트럭의 컨테이너는 현재 트럭에 담을 컨테이너보다 가벼운 것부터 탐색하면 된다.


## 1244 최대 상금
입력받은 숫자판을 M번 바꿔 받을 수 있는 최대 상금을 구하는 문제

- 모든 자리를 한번 씩 바꿔 숫자판을 바꿔주었다
- 바꿔준 숫자판을 다시 모든 자리를 한번 씩 바꿔주었다
- 이걸 M번 반복하여 바뀐 숫자판을 구했다
- 바뀐 숫자판 중 최댓값을 구했다


## 1220 Magnetic
자성체의 교착상태 개수를 구하는 문제

자성체가 놓여진 테이블을 2차원 리스트로 받을 때 세로로 계산하기 힘들다

전치행렬을 구해 가로로 계산을 한다

교착상태의 조건을 구해 개수를 구한다


## 1861 정사각형 방
최대로 이동할 수 있는 방 번호 중 제일 작은 방 번호를 구하는 문제

종료조건을 정하는게 중요함

같은 최대 이동 횟수일 때 작은 방번호를 구하는 방법이 중요함
- 최대 이동 횟수가 나왔을 때 기록해둔 방 번호를 초기화 해주는 작업이 필요


## 2817 부분수열의 합
주어진 수열의 부분수열의 합 중 타겟넘버와 같은 경우의 수를 구하는 문제

비트연산을 이용해 부분집합을 구하고 그 부분집합의 합이 타겟넘버와 같은지 확인했다
- 실행시간이 너무 오래 걸림

부분집합을 리스트로 구해서 다시 합을 구하지 않고 바로 합을 구하면서 진행하였다. 또한 부분집합에 추가될 수열이 있어도 타켓넘버를 넘어버렸다면 `break`를 사용하여 반복문을 탈출하였다


## 오목판정
오목이 맞는지 판별하는 문제

해당 방향에 원하는 개수만큼 있는지 확인하기 위해서는 `*`를 사용하면 구할 수 있다

- 다시 풀어보기


## 수영장
1일권, 1달권, 3달권, 1년권 중 1년동안 지불해야할 비용이 제일 적은 경우를 구하는 문제

처음에는 for문을 사용하여 구할려고 했지만 경우가 너무 많고 고려할 부분이 너무 많았음

완전탐색으로 바꿈

재귀함수를 사용하여 모든 경우를 구할 수 있었다

min값을 사용하여 가지치기를 추가할 수 있을 듯


## 등산로 조성
최대 갈 수 있는 등산로의 길이를 구하는 문제

최대 K까지 1번 지형을 낮출 수 있다
- 지형이 낮춰진 상태로 진행되어야함
- 최대 K까지이지 K이하로 낮춰도 된다 -> 지형의 차이만큼 낮춰주는게 제일 이득임