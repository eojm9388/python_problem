...
## 1475 방 번호
방 번호를 만들기 위한 숫자 카드 세트의 개수를 구하는 문제

6과 9는 서로 쓸 수 있다

6과 9를 한곳에 몰아 넣고, 나중에 2 나눠준다

가장 많은 숫자 개수가 필요한 세트 개수이다


## 14500 테트로미노
테트로미노를 배치했을 때 얻을 수 있는 최대 수 구하기

- `ㅜ`모양의 블럭을 제외하고는 모든 블럭을 델타탐색으로 만들 수 있다
- 4개의 블럭으로 얻을 수 있는 최대 수를 델타 탐색을 사용하여 구한다
- `ㅜ`블럭은 따로 4방향의 이동 좌표를 구해 최댓값을 구해준다. 
- 그러면 모든 블럭에서 구할 수 있는 최댓값이 구해진다.


## 14719 빗물
빗물이 고이는 부분의 개수를 구하는 문제

양쪽 끝 블록을 다 채워준다
- 왼쪽 맨위 블록이 없다면 그 줄에 블록이 있는 부분까지 다 블록으로 채운다
- 위의 방법은 왼쪽 끝 블록이 다 채워질 때까지 반복
- 오른쪽 맨위 블록도 마찬가지

위의 방법을 다 하면 빗물이 있을 수 있는 공간만 남음 